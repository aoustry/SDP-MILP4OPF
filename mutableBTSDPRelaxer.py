...
class BTSDPRelaxer():
    ...
    def buildModel(self,obj):
        ...
        self.convexAngleconstraint = set()
        
        # Parameters
        self.mutableVmin = M.parameter('vmin', self.n)
        self.mutableVmax = M.parameter('vmax', self.n)
        self.mutableVmin2 = M.parameter('vmin2', self.n)
        self.mutableVmax2 = M.parameter('vmax2', self.n)
        self.slope = M.parameter('slope',self.n)
        self.rhs_slope = M.parameter('rhs_slope',self.n)
        self.minusSinPhimin = {(b, a): M.parameter('MinusSinPhimin{0}_{1}'.format(b, a)) for (b, a) in self.ThetaMinByEdge}
        self.minusSinPhimax = {(b, a): M.parameter('MinusSinPhimax{0}_{1}'.format(b, a)) for (b, a) in self.ThetaMinByEdge}
        self.cosPhimin = {(b, a): M.parameter('cosPhimin{0}_{1}'.format(b, a)) for (b, a) in self.ThetaMinByEdge}
        self.cosPhimax = {(b, a): M.parameter('cosPhimax{0}_{1}'.format(b, a)) for (b, a) in self.ThetaMinByEdge}
        self.cosPhimean= {(b, a): M.parameter('cosPhimean{0}_{1}'.format(b, a)) for (b, a) in self.ThetaMinByEdge}
        self.sinPhimean= {(b, a): M.parameter('sinPhimean{0}_{1}'.format(b, a)) for (b, a) in self.ThetaMinByEdge}
        self.cosHaldiff = {(b, a): M.parameter('cosHaldiff{0}_{1}'.format(b, a)) for (b, a) in self.ThetaMinByEdge}
        self.__initializeModelparameters()
        #Upper level var
        ...
        for idx_clique in range(self.cliques_nbr):
            ...
            for i in range(nc):
                b = clique[i]
                M.constraint(Expr.sub(A[idx_clique].index(i,i), self.mutableVmin2.index(b)),Domain.greaterThan(0))
                M.constraint(Expr.sub(A[idx_clique].index(i,i), self.mutableVmax2.index(b)),Domain.lessThan(0))
            ...
            for i in range(nc):
                ...
                M.constraint(Expr.sub(R[idx_clique].index(0,1+i),self.mutableVmin.index(b)),Domain.greaterThan(0))
                M.constraint(Expr.sub(R[idx_clique].index(0,1+i),self.mutableVmax.index(b)),Domain.lessThan(0))
                                    
                if self.Vmax[b] > self.Vmin[b]:
                    expr_ineq = Expr.sub(Expr.mul(self.slope.index(b),R[idx_clique].index(0,1+i)),A[idx_clique].index(i,i))
                    M.constraint(Expr.add(expr_ineq,self.rhs_slope.index(b)), Domain.greaterThan(0))
                else:
                ...
                for j in range(nc):
                    ...
                    if i<j and not((clique[i],clique[j]) in already_covered):
                        ...
                        cosPhimin,minusSinPhimin, cosPhimax, minusSinPhimax, cosPhimean, sinPhimean, cosHaldiff = self.cosPhimin[(clique[i],clique[j])], self.minusSinPhimin[(clique[i],clique[j])], self.cosPhimax[(clique[i],clique[j])],self.minusSinPhimax[(clique[i],clique[j])], self.cosPhimean[(clique[i],clique[j])], self.sinPhimean[(clique[i],clique[j])], self.cosHaldiff[(clique[i],clique[j])] 
                        if phimax-phimin<=np.pi:
                            self.convexAngleconstraint.add((clique[i],clique[j]))
                            M.constraint(Expr.add(Expr.mul(minusSinPhimin,A[idx_clique].index(i,j)),Expr.mul(cosPhimin,B[idx_clique].index(i,j))),Domain.greaterThan(0))
                            M.constraint(Expr.add(Expr.mul(minusSinPhimax,A[idx_clique].index(i,j)),Expr.mul(cosPhimax,B[idx_clique].index(i,j))),Domain.lessThan(0))
                            M.constraint(Expr.sub(Expr.add(Expr.mul(cosPhimean,A[idx_clique].index(i,j)),Expr.mul(sinPhimean,B[idx_clique].index(i,j))), Expr.mul(cosHaldiff,R[idx_clique].index(1+i,1+j))),Domain.greaterThan(0))
        
                        M.constraint(Expr.add(Expr.sub(R[idx_clique].index(1+i,1+j),Expr.add(Expr.mul(self.mutableVmin.index(b), R[idx_clique].index(0,1+j)), Expr.mul( self.mutableVmin.index(a), R[idx_clique].index(0,1+i)))),Expr.mul(self.mutableVmin.index(b),self.mutableVmin.index(a))),Domain.greaterThan(0))
                        M.constraint(Expr.add(Expr.sub(R[idx_clique].index(1+i,1+j),Expr.add(Expr.mul(self.mutableVmax.index(b), R[idx_clique].index(0,1+j)), Expr.mul( self.mutableVmax.index(a), R[idx_clique].index(0,1+i)))),Expr.mul(self.mutableVmax.index(b),self.mutableVmax.index(a))),Domain.greaterThan(0))
                        M.constraint(Expr.add(Expr.sub(R[idx_clique].index(1+i,1+j),Expr.add(Expr.mul(self.mutableVmax.index(b), R[idx_clique].index(0,1+j)), Expr.mul( self.mutableVmin.index(a), R[idx_clique].index(0,1+i)))),Expr.mul(self.mutableVmax.index(b),self.mutableVmin.index(a))),Domain.lessThan(0))
                        M.constraint(Expr.add(Expr.sub(R[idx_clique].index(1+i,1+j),Expr.add(Expr.mul(self.mutableVmin.index(b), R[idx_clique].index(0,1+j)), Expr.mul( self.mutableVmax.index(a), R[idx_clique].index(0,1+i)))),Expr.mul(self.mutableVmin.index(b),self.mutableVmax.index(a))),Domain.lessThan(0))
        ...
        self.M, self.A, self.B, self.R = M, A, B, R
        
    
    def __initializeModelparameters(self):
       self.mutableVmin.setValue(np.array(self.Vmin))
       self.mutableVmax.setValue(np.array(self.Vmax))
       self.mutableVmin2.setValue(np.array(self.Vmin)**2)
       self.mutableVmax2.setValue(np.array(self.Vmax)**2)

       for i in range(self.n):
            Vmin,Vmax = self.Vmin[i], self.Vmax[i]
            if Vmin+epsilonV<Vmax:
                slope = (Vmax**2 - Vmin**2)/(Vmax-Vmin)
            else:
                slope = 0
            self.slope.index(i).setValue(slope)
            self.rhs_slope.index(i).setValue((Vmin**2)- (Vmin)*slope)
       for (b,a) in self.ThetaMaxByEdge:
            mean = 0.5*(self.ThetaMinByEdge[(b,a)] + self.ThetaMaxByEdge[(b,a)])
            halfdiff = 0.5*(self.ThetaMaxByEdge[(b,a)] - self.ThetaMinByEdge[(b,a)])
            self.minusSinPhimin[(b,a)].setValue(-np.sin(self.ThetaMinByEdge[(b,a)]))
            self.minusSinPhimax[(b,a)].setValue(-np.sin(self.ThetaMaxByEdge[(b,a)]))
            self.cosPhimin[(b,a)].setValue(np.cos(self.ThetaMinByEdge[(b,a)]))
            self.cosPhimax[(b,a)].setValue(np.cos(self.ThetaMaxByEdge[(b,a)]))
            self.cosPhimean[(b,a)].setValue(np.cos(mean))
            self.sinPhimean[(b,a)].setValue(np.sin(mean))
            self.cosHaldiff[(b,a)].setValue(np.cos(halfdiff))
        

    def updateV(self,Vmin,Vmax,i):
        assert(Vmin<=Vmax)
        assert(self.Vmin[i]-1e-7<=Vmin)
        assert(self.Vmax[i]+1e-7>=Vmax)
        self.mutableVmin.index(i).setValue(Vmin)
        self.mutableVmax.index(i).setValue(Vmax)
        self.mutableVmin2.index(i).setValue(Vmin**2)
        self.mutableVmax2.index(i).setValue(Vmax**2)
    ...
    def update_angle(self,idx_clique,b,a,phimin,phimax):
        assert(phimin<=phimax)
        assert(self.ThetaMinByEdge[(b,a)]-1e-7<=phimin)
        assert(self.ThetaMaxByEdge[(b,a)]+1e-7>=phimax)
        mean = 0.5*(phimin + phimax)
        halfdiff = 0.5*(phimax - phimin)
        self.minusSinPhimin[(b,a)].setValue(-np.sin(phimin))
        self.minusSinPhimax[(b,a)].setValue(-np.sin(phimax))
        self.cosPhimin[(b,a)].setValue(np.cos(phimin))
        self.cosPhimax[(b,a)].setValue(np.cos(phimax))
        self.cosPhimean[(b,a)].setValue(np.cos(mean))
        self.sinPhimean[(b,a)].setValue(np.sin(mean))
        self.cosHaldiff[(b,a)].setValue(np.cos(halfdiff))
        if phimax-phimin<=np.pi:
            if not((b,a) in self.convexAngleconstraint):
                clique = self.cliques[idx_clique]
                i,j = clique.index(b),clique.index(a)
                self.convexAngleconstraint.add((b,a))
                cosPhimin,minusSinPhimin, cosPhimax, minusSinPhimax, cosPhimean, sinPhimean, cosHaldiff = self.cosPhimin[(b,a)], self.minusSinPhimin[(b,a)], self.cosPhimax[(b,a)],self.minusSinPhimax[(b,a)], self.cosPhimean[(b,a)], self.sinPhimean[(b,a)], self.cosHaldiff[(b,a)] 
                self.M.constraint(Expr.add(Expr.mul(minusSinPhimin,self.A[idx_clique].index(i,j)),Expr.mul(cosPhimin,self.B[idx_clique].index(i,j))),Domain.greaterThan(0))
                self.M.constraint(Expr.add(Expr.mul(minusSinPhimax,self.A[idx_clique].index(i,j)),Expr.mul(cosPhimax,self.B[idx_clique].index(i,j))),Domain.lessThan(0))
                self.M.constraint(Expr.sub(Expr.add(Expr.mul(cosPhimean,self.A[idx_clique].index(i,j)),Expr.mul(sinPhimean,self.B[idx_clique].index(i,j))), Expr.mul(cosHaldiff,self.R[idx_clique].index(1+i,1+j))),Domain.greaterThan(0))

 
    def computeBTminAngle(self,idx_clique_obj,bobj,aobj):
